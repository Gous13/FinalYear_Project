...
import models # Important: used to register all models
...
def seed():
    with app.app_context():
        ...
        # Use models package to avoid import context issues
        QuestionModel = models.AssessmentQuestion
        
        # Clear existing
        try:
            QuestionModel.query.filter_by(skill_name='SQL').delete()
        except Exception as e:
            print(f"Cleanup failed: {e}")

        # EASY QUESTION
        easy_q = QuestionModel(
            skill_name='SQL',
            difficulty='easy',
        ...
        db.session.commit()
        print(f"SQL Questions successfully seeded in {db_path}")
