...
const SkillsInput = ({ value, onChange, placeholder = 'Type to search skills (e.g., sql, react)...' }) => {
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="min-h-[80px] px-3 py-2 border border-gray-300 rounded-md focus-within:ring-2 focus-within:ring-primary-500 focus-within:border-primary-500 bg-white">
        ...
        <input
          ...
          className="w-full border-0 p-0 focus:ring-0 focus:outline-none text-gray-900 placeholder-gray-400"
        />
      </div>
      {/* Suggestions dropdown */}
      {showSuggestions && suggestions.length > 0 && (
        <ul className="absolute z-10 mt-1 w-full max-h-48 overflow-auto bg-white border border-gray-200 rounded-md shadow-lg py-1">
          {suggestions.map((skill, idx) => (
            <li
              ...
              className={`px-3 py-2 cursor-pointer text-sm ${
                idx === highlightedIndex ? 'bg-primary-50 text-primary-800' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              {skill}
            </li>
