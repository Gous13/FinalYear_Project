    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {STATUS_OPTIONS.map(({ value, label }) => (
          <div key={value} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
            <div className="font-medium text-gray-700 mb-2">{label}</div>
            <div className="space-y-2">
              {byStatus[value].map((task) => (
                <div key={task.id} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
